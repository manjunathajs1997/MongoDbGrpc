...
public class ErrorHandlingClient {
  ...
  void blockingCall() {
    ...
    try {
      stub.sayHello(HelloRequest.newBuilder().setType("Bart").build());
    } catch (Exception e) {
  ...
  void futureCallDirect() {
    ...
    ListenableFuture<HelloReply> response =
        stub.sayHello(HelloRequest.newBuilder().setType("Lisa").build());
  ...
  void futureCallCallback() {
    GreeterFutureStub stub = GreeterGrpc.newFutureStub(channel);
    ListenableFuture<HelloReply> response =
        stub.sayHello(HelloRequest.newBuilder().setType("Maggie").build());
  ...
  void asyncCall() {
    ...
    HelloRequest request = HelloRequest.newBuilder().setType("Homer").build();
    final CountDownLatch latch = new CountDownLatch(1);
  ...
  void advancedAsyncCall() {
    ...
    call.sendMessage(HelloRequest.newBuilder().setType("Marge").build());
    call.halfClose();
